    ...
    if mood == "Mutlu":
        ...
        if filtered_songs is not None:
            filtered_songs = filtered_songs[
                (filtered_songs['valence_%'] >= 60) &
                (filtered_songs['energy_%'] >= 60) &
                (filtered_songs['danceability_%'] >= 50)
            ]
        if filtered_movies is not None:
            filtered_movies = filtered_movies[
                filtered_movies['listed_in'].str.contains("Comedy|Musical|Children", case=False, na=False)
            ]
        if filtered_books is not None:
            filtered_books = filtered_books[
                filtered_books['Book-Title'].str.contains("Happy|Joy|Funny|Comedy|Joke|Laugh", case=False, na=False)
            ]

    elif mood == "Üzgün":
        if filtered_songs is not None:
            filtered_songs = filtered_songs[
                (filtered_songs['valence_%'] <= 40) &
                (filtered_songs['energy_%'] <= 40)
            ]
        if filtered_movies is not None:
            filtered_movies = filtered_movies[
                filtered_movies['listed_in'].str.contains("Drama|Romance", case=False, na=False)
            ]
        if filtered_books is not None:
            filtered_books = filtered_books[
                filtered_books['Book-Title'].str.contains("Sad|Cry|Death|Grief|Tears|Sorrow", case=False, na=False)
            ]
        ...
        if filtered_movies is not None:
            filtered_movies = filtered_movies[
                filtered_movies['listed_in'].str.contains("Action|Thriller|Horror", case=False, na=False)
            ]
        if filtered_books is not None:
            filtered_books = filtered_books[
                filtered_books['Book-Title'].str.contains("Anger|War|Fight|Kill|Murder|Blood", case=False, na=False)
            ]

    elif mood == "Rahat":
        if filtered_songs is not None:
            filtered_songs = filtered_songs[
                (filtered_songs['energy_%'] <= 50) &
                (filtered_songs['valence_%'] >= 40) &
                (filtered_songs['acousticness_%'] >= 40)
            ]
        if filtered_movies is not None:
            filtered_movies = filtered_movies[
                filtered_movies['listed_in'].str.contains("Documentary|Nature|Music", case=False, na=False)
            ]
        if filtered_books is not None:
            filtered_books = filtered_books[
                filtered_books['Book-Title'].str.contains("Calm|Peace|Relax|Yoga|Meditation|Zen", case=False, na=False)
            ]
