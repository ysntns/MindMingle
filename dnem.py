...
@st.cache_data
def prepare_spotify_features(data):
    features = data[['danceability_%', 'energy_%', 'valence_%', 'acousticness_%', 'instrumentalness_%', 'liveness_%', 'speechiness_%']]
    scaler = MinMaxScaler()
...
# Load data
netflix_data = load_netflix_data('netflix.csv')
spotify_data = load_spotify_data('spotify.csv')
...
if spotify_data is None:
    st.error("Spotify data file not found.")
    st.stop()

# Prepare features
spotify_normalized_features = prepare_spotify_features(spotify_data)
