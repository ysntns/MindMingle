...
def filter_contents(data, mood):
    if mood == "Çok Mutlu" or mood == "Mutlu":
        filtered_data = data[data['listed_in'].str.contains("Comedy|Animation", regex=True)]
    elif mood == "Üzgün":
        filtered_data = data[data['listed_in'].str.contains("Drama|Romantic|Comedy", regex=True)]
    elif mood == "Keyifli":
        filtered_data = data[data['listed_in'].str.contains("Family|Documentary|Animation", regex=True)]
    elif mood == "Melankolik":
        filtered_data = data[data['listed_in'].str.contains("Art House|Independent|Drama", regex=True)]
...
st.sidebar.image(r"logo.png", use_column_width=True)
